...
export default function Technologies() {
  ...
  return (
    <section id="tecnologias" className="py-24 bg-dark relative overflow-hidden">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          {techStack.map((stack, index) => {
            ...
            return (
              <div
                key={index}
                className={`bg-dark-lighter p-8 rounded-xl border ${stack.borderColor} hover:border-opacity-100 transition-all duration-300 card-hover`}
              >
                <div className="flex items-center gap-4 mb-6">
                  <div className="p-3 bg-dark rounded-lg">
                    <Icon className={`w-8 h-8 ${stack.color}`} />
                  </div>
                  <h3 className="text-2xl font-bold text-gray-100">{stack.category}</h3>
                </div>
                ...
                <div className="space-y-4">
                  {stack.technologies.map((tech, i) => (
                    <div
                      key={i}
                      className="group p-4 bg-dark rounded-lg border border-dark-border hover:border-primary/30 transition-all duration-300 cursor-pointer"
                    >
                      <div className="flex items-start justify-between">
                        <div>
                          <h4 className="font-bold text-gray-100 mb-1 group-hover:text-primary transition-colors">
                            {tech.name}
                          </h4>
